...
### ✅ 20. 端到端测试 → 剧本 P0 修复 (2026-06-30)

首次端到端测试（完整五章用户交互）暴露六项问题，P0 修复完成：

**P0a — 判决章被跳过**：三个判决章末尾的 `（表演结束。）` 和 ⛔ 区的
"表演自然结束"被 Ghost 误读为"停止输出"指令，导致 52 行判词完全未执行。
→ 移除全部 `（表演结束。）`，⛔ 区改为"本章为终章。输出判词后不调任何
推进命令"。

**P0b — 安全阀失效 + 追问质量差**：原"追问一次"在测试中实际跑了 5 轮，
追问全是开放式（"什么事？""说清楚。"），没有产生有效信息推动剧情。
→ 改为"追问最多两次，必须用闭合式问题（给 2-3 个有限选项）"，每章加
好的追问示例 vs 坏的追问示例，`.meta.md` 全局 nav_rule 同步更新。

**P0c — 计时器全部缺失**：计时器功能已构建但从未在剧本中使用。
→ 每个交互章的封闭式问题前加 `<set_timer_state t="running:15"/>`，
`.meta.md` 全局规则"每次提问前设 15 秒计时器"。

| 文件 | 改动 |
|------|------|
| `06-acquittal.md` | 去 `（表演结束。）` + ⛔ 重写 |
| `06-lenient.md` | 去 `（表演结束。）` + ⛔ 重写 |
| `06-guilty.md` | 去 `（表演结束。）` + ⛔ 重写 |
| `02-motive.md` | 安全阀闭合式追问 + 计时器 |
| `03-dispute.md` | 安全阀闭合式追问 + 计时器 + 路径条件细化 |
| `03-friend.md` | 新增安全阀（原缺失）+ 计时器 |
| `04-knife.md` | 安全阀闭合式追问 + 计时器 |
| `05-reversal.md` | 计时器 |
| `.meta.md` | nav_rule 更新（追问最多两次+闭合式+计时器） |

待下次测试验证。

### ✅ 21. 第二次 E2E 测试 → 判决章防改写 + <_> 禁令 (2026-06-30)

第二次全链路测试暴露两个新 P0：

**P0a — 判决章被 Ghost 完全重写**：Ghost 进入 06-lenient 后忽略全部 52 行
剧本，即兴编了"过失致人死亡。防卫过当。三年。"然后加 `jump_chapter id="guilty"`，
导致用户看到两个矛盾判决（先三年后死刑）。

| 文件 | 改动 |
|------|------|
| `06-acquittal.md` | ⛔ 重写为死禁：逐句照读、禁 jump_chapter、禁追加评论 |
| `06-lenient.md` | 同上 |
| `06-guilty.md` | 同上 |

**P0b — `<_>` 包裹标签已成习得行为**：Ghost 几乎每组 CTML 命令都用
`<_>...</_>` 包裹，消耗上下文且表明对 CTML 语法的系统性误认。

| 文件 | 改动 |
|------|------|
| `.meta.md` | 新增 `ctml_grammar` 规则：`<_>` 是语法错误，命令独立写不加包裹 |

待第三次测试验证：判决章是否逐句照读、是否不再调用 jump_chapter、`<_>` 是否消失。

---

*Created: 2026-06-28. Updated: 2026-06-30 — Tasks 13-21 完成。*
